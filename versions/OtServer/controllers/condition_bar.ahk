LocateConditionBar() {
	...
	ArColor.Conditions := {	EatFood: CorToDec("#F6D48F"),			Haste: CorToDec("#B08B50"),			EnergyRing: CorToDec("#272DCC"),	LogoutBlock: CorToDec("#7E7E83")
		...
		,					Root: CorToDec("#573C36"), 			CureParalyze2: CorToDec("#FF0000")}
}
...
Verify_Conditions(){
	...
	if (TibiaServer = "Shadow-Illusion") {
		...
		; Offsets to check: center first, then alternating left/right
		; Order: 0, -1, +1, -2, +2, -3, +3
		PixelOffsets := [0, -1, 1, -2, 2, -3, 3]

		loop, 7 {
			;~ Crop_And_Save_Haystack(1x,1y,50,50)
			Found_Condition := false

			for OffsetIdx, OffsetVal in PixelOffsets {
				...
				for ConditionsName, ConditionsCor in ArColor.Conditions {
					If (ConRead[A_Index] != A_Index)
						If (Color = ConditionsCor){
							;~ Msgbox % "ConditionsName: " ConditionsName "`nColorDec: " Color "`nOffset: " OffsetVal
							Data.Conditions[ConditionsName] := 1
							ConRead[A_Index] := A_Index
							Found_Condition := true
							1x := 1xk	; Update base position for next slots
							Break 2
						}
				}
			}

			; Only reset conditions to 0 if NO offset found a match for this slot
			if (Found_Condition != true) {
				for ConditionsName, ConditionsCor in ArColor.Conditions {
					If (ConRead[A_Index] != A_Index)
						Data.Conditions[ConditionsName] := 0
